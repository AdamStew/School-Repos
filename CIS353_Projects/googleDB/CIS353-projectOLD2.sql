...
FROM Shadow_Copy SC, Document D, Directory DY, UserTbl U, WebApp W
WHERE D.ID = SC.DocID
      AND DY.ID = D.DirID
...
-- Every directory who doesn't have a parent, but has all directories that were created after 2009
...
WHERE NOT EXISTS(
	...
	SELECT D.ID, D.Name
	FROM Directory D
	WHERE D.DateCreated <= '01-01-2005 12:00 AM' AND D.parent IS NOT NULL
);
