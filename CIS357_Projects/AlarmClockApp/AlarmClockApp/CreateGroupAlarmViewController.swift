//
//  CreateGroupAlarmViewController.swift
//  AlarmClockApp
//
...
protocol CreateGroupAlarmViewControllerDelegate {
    func groupAlarmDelegation(alarm: Alarm)
}


class CreateGroupAlarmViewController: AlarmViewController, CreateGroupAlarmTableViewControllerDelegate {
    ...
    var gameType : String?
    var friends : [String]?
    var delegate : CreateGroupAlarmViewControllerDelegate?
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if segue.identifier == "alarmInfoSegue" {
            if let dest = segue.destination as? CreateGroupAlarmTableViewController {
                dest.delegate = self
                if vcStatus == "Edit" {
                    var temp : String
                    ...
                    for friend in (self.editAlarm?.friends)! {
                        allFriends += friend
                        allFriends += ", "
                    }
                    allFriends = allFriends.substring(to: allFriends.index(before: allFriends.endIndex)) //Gets rid of last space.
                    allFriends = allFriends.substring(to: allFriends.index(before: allFriends.endIndex)) //Gets rid of last comma.
                    
                    dest.alarmNameStr = self.editAlarm?.name
                    dest.repeatStr = temp
                    dest.ringtoneStr = self.editAlarm?.ringtone
                    dest.gameStr = self.editAlarm?.gameType
                    dest.friendsStr = allFriends
                }
            }
        }
    }
    ...
    override func viewWillDisappear(_ animated: Bool) {
        ...
        if status == "Save" && vcStatus != "Edit" {
            ...
            self.newAlarm = Alarm(hour: hour, minute: minute, timeOfDay: timeOfDay, gameType: gameType!, daysOfWeek: daysOfWeek!, repeating: repeating, name: alarmName!, ringtone: ringtone!, active: true, friends: friends!)
            self.delegate?.groupAlarmDelegation(alarm: self.newAlarm!)
            ...
            self.editAlarm = Alarm(hour: hour, minute: minute, timeOfDay: timeOfDay, gameType: gameType!, daysOfWeek: daysOfWeek!, repeating: repeating, name: alarmName!, ringtone: ringtone!, active: true, friends: friends!)
            self.delegate?.groupAlarmDelegation(alarm: self.editAlarm!)
        }
    }
