...
class cryptotest{
    ...
    public static void main(String args[]){
		...
		c.setPrivateKey("RSApriv.der"); //Get private key.
		c.setPublicKey("RSApub.der"); //Get public key.
		SecretKey s = c.generateAESKey(); //Get secret key.
		byte encryptedsecret[] = c.RSAEncrypt(s.getEncoded()); //Encrypt secret key using public key.
		SecureRandom r = new SecureRandom();
		byte ivbytes[] = new byte[16];
		r.nextBytes(ivbytes);
		IvParameterSpec iv = new IvParameterSpec(ivbytes);
		String plaintext = "This is a test string to encrypt"; //Test message.
		byte ciphertext[] = c.encrypt(plaintext.getBytes(),s,iv); //Encrypt message with our secret key and IV.
		System.out.printf("CipherText: %s%n",DatatypeConverter.printHexBinary(ciphertext));
		byte decryptedsecret[] = c.RSADecrypt(encryptedsecret); //Decrypt secret key using private key.
		SecretKey ds = new SecretKeySpec(decryptedsecret,"AES");
		byte decryptedplaintext[] = c.decrypt(ciphertext,ds,iv); //Decrypt message using their secret key and IV.
